...
export type SSEEvent =
  ...
  | { type: 'score-update'; answerId: string; scores: { relevance: number; depth: number; clarity: number; overall: number } }
  ...
  const connect = useCallback(() => {
    ...
    const handle = (eventName: string, e: MessageEvent) => {
      try {
        ...
        const VALID_TYPES = new Set(['connected','scoring-start','score-update','scoring-error','follow-up','speech-event','timer-tick','session-ended'])
    ...
    const events: SSEEvent['type'][] = [
      ...
      'follow-up', 'speech-event', 'timer-tick', 'session-ended',
    ]
