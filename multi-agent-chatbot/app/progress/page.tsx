...
export default function ProgressPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      progressApi.timeline().then(d => setTimeline(d.timeline)),
    ]).finally(() => setLoading(false))
  }, [])

  // Flatten timeline into chart data points
  const chartData = timeline.flatMap(session =>
    session.observations
      .filter((o: any) => o.type === 'technical_accuracy' && typeof o.score === 'number')
      .map((o: any) => ({
        date: new Date(o.timestamp).toLocaleDateString('en', { month: 'short', day: 'numeric' }),
        score: o.score,
        concept: o.concept,
      }))
  ).slice(-20)
