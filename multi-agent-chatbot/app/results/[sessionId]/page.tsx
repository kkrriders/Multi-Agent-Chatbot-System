...
import { interview as interviewApi, type Answer } from '@/lib/api'
import { Nav } from '@/components/nav'
import { CheckCircle, ChevronDown, ChevronUp, Mic, Trophy, ArrowRight, RotateCcw } from 'lucide-react'
...
export default function ResultsPage() {
  ...
  useEffect(() => {
    interviewApi.summary(sessionId).then(d => { setData(d); setLoading(false) })
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          ...
          {interview.categoryScores && Object.keys(interview.categoryScores).length > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-8">
              {Object.entries(interview.categoryScores).map(([cat, s]: any) => (
          ...
          {data.progress && (
            <div className="border border-border rounded-xl p-4 mb-6 space-y-3">
              ...
              {data.progress.weakAreas?.length > 0 && (
                <div>
              ...
              {data.progress.strongAreas?.length > 0 && (
                <div>
