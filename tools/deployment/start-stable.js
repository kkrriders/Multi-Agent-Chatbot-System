...
const { getDynamicOllamaURL } = require('./shared/wsl-network');
...
async function checkOllamaConnection() {
  ...
  try {
    const ollamaBase = await getDynamicOllamaURL();
    ...
    console.error(`   - WSL2 network connectivity is working`);
    return false;
  }
}
...
async function startServices() {
  ...
  const services = [
    { name: 'MONITOR', script: 'enhanced-performance-monitor.js', color: colors.cyan },
